...
export const convertNoteToFlowNode = (note: NoteData, onUpdate: (id: string, content: string) => void): Node => {
  ...
  const baseData = {
    ...
    concept: undefined, // NoteData에는 없음
    source: undefined, // NoteData에는 없음  
  ...
  switch (note.type) {
    ...
    case '유형_레버':
      ...
      nodeData = { ...baseData, basis: note.basis } as TangibleLeverNodeData;
      break;
    case '무형_레버':
      nodeType = 'intangible_lever';
      nodeData = { ...baseData, basis: note.basis } as IntangibleLeverNodeData;
      break;
    default:
...
export const convertFlowNodeToNote = (node: Node): NoteData => {
  ...
  return {
    ...
    basis: 'basis' in data && data.basis ? {
      author: '',
      theory: data.basis,
      year: ''
    } : undefined,
...
export const convertConnectionToFlowEdge = (connection: ConnectionData): Edge => {
  return {
    ...
    type: connection.relationType === 'direct' ? 'default' : 'step',
    animated: connection.relationType === 'direct',
    style: {
      strokeWidth: 2,
      stroke: connection.isPositive !== false ? '#10b981' : '#ef4444',
    },
    markerEnd: {
      type: 'arrowclosed',
      width: 20,
