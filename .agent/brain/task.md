# Task List: 레이어 높이 UI 버그 수정 + 레이어 로직 리팩토링
...
## Task 1: 레이어 높이 상한 1000 통일
- [x] 슬라이더/숫자 입력 상한 1000 통일
- [x] 자동 계산/드래그 확장 상한 1000 통일

## Task 2: 레이어 UI 조절 시 노드 이동 제거
- [x] 레이어 높이 변경에 따른 노드 Y 이동 로직 차단
...
# Task List: 선택 영역 일괄 고정/해제 + 컨텍스트 메뉴 클램프
...
## Task 1: 선택 영역 일괄 고정/해제
- [x] 선택 노드 상태 유지 및 컨텍스트 메뉴 섹션 추가
- [x] 다중 pin/unpin 처리 시 잠금 노드 제외

## Task 2: 컨텍스트 메뉴 클램프 개선
- [x] 메뉴 위치 clamp 적용
- [x] 우클릭 시 뷰포트 이동 제거
- [x] 섹션 토글 시 재클램프

## Task 3: 검증
- [ ] 선택 영역 우클릭에서 전체 고정/해제 동작 확인
- [ ] 화면 가장자리 우클릭 시 메뉴 잘림 없이 표시 확인


# Task List: 직전 작업 Undo (공유 히스토리)
...
## Task 1: LiveblocksService 공유 히스토리
- [x] Yjs 히스토리 스택 저장
- [x] undo 시 마지막 스냅샷 복원

## Task 2: undo_layout 경로 개선
- [x] undo_layout에서 UndoManager 우선 실행
- [x] undo 히스토리 없을 때 pre-layout 스냅샷 fallback 유지

## Task 3: 검증
- [ ] 노드 이동/추가/삭제 후 “이전상태”로 직전 작업 복원 확인
- [ ] 히스토리 없을 때 auto_layout 스냅샷 복원 동작 확인
...
## Task 1: Liveblocks 공유 스냅샷
- [x] 스냅샷 저장/목록/조회 API 추가

## Task 2: 저장/로드 경로 전환
- [x] save_snapshot/restore_snapshot에서 공유 스냅샷 우선 적용
- [x] ExportMenu 목록 새로고침에서 공유 스냅샷 우선 적용
