# Implementation Plan: 레이어 높이 UI 버그 수정 + 레이어 로직 리팩토링

## 목표
1. 레이어 높이 UI 조절 시 노드 전체 이동/무한 확장 버그 제거
2. 레이어 높이 상한을 1000으로 통일
3. 레이어 배경/패널 로직 분리로 유지보수성 개선
...
### 1) 레이어 높이 상한 통일
- 슬라이더/숫자 입력/자동 계산/드래그 확장 상한을 1000으로 일관화

### 2) 레이어 UI 조절 시 노드 이동 제거
- 레이어 높이 변경 시 노드 Y 이동(useEffect 리플로우) 차단
- 자동 높이 재계산은 유지하되 UI 조절 경로에서는 노드 위치 유지

### 3) 레이어 로직 리팩토링
- 레이어 배경/라벨 렌더를 별도 컴포넌트로 분리
- 레이어 컨트롤 패널을 별도 컴포넌트로 분리
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| 레이어 높이 변경 후 노드가 밴드 밖에 위치 | 시각적 이탈 | 자동 높이 재계산/드래그 클램프 유지 |
| 분리 컴포넌트 간 상태 누락 | UI 오류 | 핸들러/props 명시적 전달 |
...
### 수동 검증
1. 레이어 높이 슬라이더/숫자 입력 조절 시 노드 위치가 유지되는지 확인
2. 상한 1000이 슬라이더/숫자 입력/드래그 확장에 동일 적용되는지 확인
3. 레이어 배경/패널 UI 정상 동작 확인
...
# Implementation Plan: 선택 영역 일괄 고정/해제 + 컨텍스트 메뉴 클램프

## 목표
1. 선택 영역 내 노드를 우클릭 메뉴에서 일괄 고정/고정해제 가능하게 구현
2. 우클릭 메뉴가 화면 가장자리에서 잘리지 않도록 위치만 클램프하고 뷰포트 이동 제거
3. 메뉴 섹션 토글로 높이가 변경될 때도 다시 클램프
...
### 1) 선택 영역 일괄 고정/해제
- onSelectionChange로 선택 노드 목록 유지
- 컨텍스트 메뉴에 "선택 노드" 섹션 추가
- 다중 노드 pin/unpin 처리 시 잠금된 노드는 제외

### 2) 컨텍스트 메뉴 클램프
- 메뉴의 좌표를 화면 내로 clamp
- setViewport로 화면 이동하는 로직 제거
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| 선택 노드 중 잠금 노드 포함 | 일부 노드만 고정/해제 | 잠금된 노드는 제외 처리 |
| 메뉴 확장 시 화면 밖으로 벗어남 | 일부 메뉴 잘림 | 섹션 토글 시 재클램프 |
...
### 수동 검증
1. 드래그 선택 후 우클릭 메뉴에서 "전체 고정"/"전체 고정해제" 동작 확인
2. 화면 가장자리에서 우클릭 시 메뉴가 잘리지 않고 위치만 조정되는지 확인
3. 메뉴 섹션을 펼쳐도 화면 밖으로 벗어나지 않는지 확인

---

# Implementation Plan: 직전 작업 Undo (공유 히스토리)

## 목표
1. “이전상태” 버튼이 참여자 구분 없이 직전 작업을 되돌리도록 공유 히스토리 도입
2. 협업 세션 전체의 마지막 변경을 순서대로 되돌림
3. 기존 auto_layout 스냅샷 복원은 fallback으로 유지

---

## 핵심 변경 범위

### 1) LiveblocksService 공유 히스토리
- nodes/connections/layerSettings 스냅샷을 Yjs 배열에 저장
- undo 시 마지막 스냅샷 복원

### 2) undo_layout 경로 개선
- undo_layout 요청 시 UndoManager 우선 실행
- undo 히스토리 없을 때만 pre-layout 스냅샷 복원
...
### 수동 검증
1. 노드 이동/추가/삭제 후 “이전상태” 버튼으로 직전 작업 복원
2. undo 히스토리 없을 때 기존 auto_layout 스냅샷 복원 동작

---

# Implementation Plan: 스냅샷 전역 공유

## 목표
1. 스냅샷 저장/로드를 Liveblocks 기반으로 전역 공유
2. 비연결 시 로컬 스냅샷 유지
...
### 1) LiveblocksService 공유 스냅샷
- yDoc map에 스냅샷 저장
- 스냅샷 목록 조회 API 제공

### 2) 저장/로드 경로 전환
- save_snapshot/restore_snapshot에서 공유 스냅샷 우선 사용
- ExportMenu 목록 새로고침에서 공유 스냅샷 우선 사용
...
### 수동 검증
1. A가 저장한 스냅샷이 B/C/D 목록에 표시되는지 확인
2. B가 복원하면 모든 참여자 화면에 동일하게 반영되는지 확인
...
# Implementation Plan: 레이어 높이/노드 위치 동기화 스팸 차단

## 목표
1. updateStickyNote 반복 호출 감소
2. 자동/원격 레이어 높이 변경 시 노드 위치 재동기화 차단
3. 드래그 종료 시에만 노드 위치 동기화
...
### 1) 레이어 높이 변경 출처 구분
- 사용자 조작 시에만 노드 위치를 Liveblocks로 배치 동기화
- 자동 계산/원격 복원은 동기화하지 않음

### 2) 드래그 종료 동기화
- drag 종료된 노드만 updateStickyNote 수행
- 프로그램적 변경은 Liveblocks로 재전파하지 않음
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| 사용자 레이어 높이 변경이 원격에 반영되지 않음 | 뷰 불일치 | 사용자 조작 flag로 배치 동기화 유지 |
| 드래그 외 변경이 공유되지 않음 | 위치 누락 | 드래그 종료에만 제한, 필요 시 별도 액션에서 동기화 |
...
### 수동 검증
1. 새로고침 후 updateStickyNote 스팸 로그 감소 확인
2. 노드 드래그 종료 시에만 위치 동기화 확인
3. 레이어 높이 자동 재계산 중 노드 재동기화가 발생하지 않는지 확인
