...
### 6. 전역 패널 스타일 충돌 방지
```css
/* ❌ 전역 .left-panel button 같은 범용 스타일은 다른 컴포넌트 버튼을 망가뜨림 */
/* ✅ 컴포넌트 루트로 스코프를 좁혀 충돌 방지 */
.ai-chat-sidebar .footer-icon-btn { width: 32px; }
```

권장 점검:
1. 전역 컨테이너(.left-panel)에서 button/p 요소를 강제 스타일링하지 않는지 확인
2. 필요 시 컴포넌트 루트(예: .ai-chat-sidebar)로 명시적으로 스코프 제한
...
- [ ] 전역 컨테이너(.left-panel) 스타일이 컴포넌트 버튼을 덮어쓰지 않는가?
